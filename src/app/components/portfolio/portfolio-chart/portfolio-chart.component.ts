...
export class PortfolioChartComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.container = this.elRef.nativeElement.querySelector('#chart');
    const width = 600;
    const height = 300;
    ...
    function businessDayToString(businessDay): string {
    ...
    chart.subscribeCrosshairMove((param) => {
      ...
      toolTip.style.width = toolTipWidth.toString();
      const price = Number(param.seriesPrices.get(series));
      toolTip.innerHTML = '<div style="color: #ffff00">⬤ AERO</div>' +
        '<div style="font-size: 24px; margin: 4px 0; color: #ffff00">' + (Math.round(price * 100) / 100).toFixed(2) + '</div>' +
        '<div>' + dateStr + '</div>';

      let left = Number(param.point.x) - (toolTipWidth / 2);
      if (left > width - toolTipWidth - priceScaleWidth) {
        left = width - toolTipWidth - priceScaleWidth;
      } else if (left < (toolTipMargin / 2)) {
        left = 0;
      }

      toolTip.style.left = left + 'px';
      // adjust depending on where it sits on the page later on
